...
class FastIndex {
  constructor(opts) {
    ...
    this.removeNonWord =
      (typeof opts.removeNonWord === 'boolean')
      ? opts.removeNonWord
      : true;
  ...
  build(key, spellingsFn){
    ...
    if (this.removeNonWord) {
      key = removeNonWord(key);
    }
